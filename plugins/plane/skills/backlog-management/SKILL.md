...
Tools below are referenced by their **action name** (e.g., `list_work_items`). The actual MCP tool name follows a pattern like `mcp__<provider>__plane-i-<action>`. Discover the exact prefix by listing available tools, then match by action suffix.
