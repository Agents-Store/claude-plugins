---
name: task-execution
description: Triggering tasks, monitoring runs, debugging failures, and managing run lifecycle. Use when triggering tasks, inspecting runs, checking run status, or debugging failures.
---
...
→ Task list with IDs and payload schemas
```
...
```
Tool: tds-list_runs
Input: {"environment": "prod", "limit": 20}
```
...
```
Tool: tds-list_runs
Input: {"taskIdentifier": "process-order", "environment": "prod", "period": "1d"}
```
...
```
Tool: tds-list_runs
Input: {"tag": "vip-customer", "environment": "prod"}
```
...
→ Returns current state when completed or timeout
```
...
docker compose logs webapp --tail=20 | grep "task"
```
...
| Issue | Cause | Fix |
|-------|-------|-----|
| PENDING_VERSION forever | Worker not connected to correct environment | Check worker config and TRIGGER_SECRET_KEY |
| SYSTEM_FAILURE | Worker crashed or OOM | Increase Docker memory limits |
| All runs CRASHED | Docker container restarting | Check `docker compose ps` for unhealthy containers |
| Runs stuck in QUEUED | Worker not processing | Restart worker: `docker compose restart worker` |
