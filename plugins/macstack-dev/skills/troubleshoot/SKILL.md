...
| A cross-reference check reports a missing ID that is visibly present | a Cyrillic homoglyph in the ID token (`К` vs `K`, also `А О С Е`) → rewrite the token in ASCII |
